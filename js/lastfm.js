...
angular.module('lastfm').factory('lastFMService', ['$http', '$q',
  function ($http, $q) {
    "use strict";
    var lastFMService = {
        ...
        var promise = $q.all([p1, p2]).then(function (d) {
          ...
          for (var i = 0; i < d.length; i++) {
            for (var j = 0; d[i].data.albums && d[i].data.albums.album && j < d[i].data.albums.album.length; j++) {
              findings.push({
                'artist': d[i].data.albums.album[j].artist.name,
                'album': d[i].data.albums.album[j].name,
                'image': d[i].data.albums.album[j].image[2]['#text']
              });
            }
          }
