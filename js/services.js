angular.module('swl').factory('watchListService', ['dropboxService', '$log',
  function (dropboxService, $log) {
    "use strict";
      ...
      getData: function (cb) {
        if (dropboxService.isAuth()) {
          ...
          dropboxService.get(function (data) {
            ...
            $log.error('Error opening default datastore: ' + error);
          });
        } else {
  ...
  function (linguaService, watchListService, spotifyService, lastFMService, statusService, swlSettings, $log) {
    ...
    var artistNewsModelService = {
      ...
      populate: function () {
        ...
        var handler = function (findings, i) {
      ...
      addNews: function (a) {
        if (artistNewsModelService.containsNews(a)) {
          ...
          artistNewsModelService.artistNewsModel.news.push({
            "artist": a,
            "added": watchListService.dformat()
          });
    ...
    var artistAlbumModelService = {
      ...
      addArtistAlbum: function (ar, al) {
        if (artistAlbumModelService.containsArtistAlbum(ar, al)) {
          ...
          artistAlbumModelService.artistAlbumModel.artistAlbums.push({
            "artist": ar,
            "album": al,
            "added": watchListService.dformat()
          });
...
angular.module('swl').factory('statusService', ['$timeout',
  function ($timeout) {
    "use strict";
    var statusService = {
      statuses: [],
      add: function (type, msg) {
        statusService.statuses.push({
          'type': type,
          'msg': msg
        });
        $timeout(function () {
